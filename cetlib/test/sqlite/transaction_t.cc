...
namespace {
  ...
  vector<array<string,2u>> const names {{"Cindy", "Miller"}, {"Billy", "Baker"}};

  void check_table_contents(Connection& c, string const& table_name)
  {
...
int main()
  try {
    ...
    create_table(c, table_name, column<string>{"FirstName"}, column<string>{"LastName"});
    {
      Transaction txn {c};
    ...
    create_table(c, table_name2, column<string>{"FirstName"}, column<string>{"LastName"});
    ...
    // Test select before commit is called
    {
      ...
      // Calling a SELECT before the transaction is committed implicitly ends the transaciton.
      check_table_contents(c, table_name2);
      txn.commit(); // Call commit to avoid the error emitted from the ~Transaction().
    }
