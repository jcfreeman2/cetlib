...
namespace {
  void compare_statements(std::string test, std::string ref)
  {
  ...
  try
    {
      ...
      // One column
      {
        auto const& test = detail::create_table_ddl("employees", column<string>{"name"});
      ...
      // Two columns
      {
        auto const& test = detail::create_table_ddl("employees", column<string>{"name"}, column<int>{"id"});
        std::string const ref {detail::create_table("employees") + "(name text, id integer)"};
        compare_statements(test, ref);
      }

      // Two columns, the first with the PRIMARY KEY constraint
      {
        auto const& test = detail::create_table_ddl("employees", column<int, primary_key>{"id"}, column<string>{"name"});
        std::string const ref {detail::create_table("employees") + "(id integer PRIMARY KEY, name text)"};
        compare_statements(test, ref);
      }
