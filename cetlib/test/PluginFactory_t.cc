...
{
  BOOST_REQUIRE_EQUAL("TestPluginBase"s, PluginTypeDeducer<cettest::TestPluginBase>::value);
...
{
  auto p = pf.makePlugin<std::unique_ptr<cettest::TestPluginBase>, std::string>("TestPlugin", "Hi");
  ...
  BOOST_CHECK_EXCEPTION(pf.makePlugin<std::unique_ptr<cettest::TestPluginBase> >("TestPluginX"s, "Hi"s),
                        ...
                        [](cet::exception const& e) {
                          return e.category() == "Configuration" &&
                            std::string{e.what()}.find("ETERNAL") != std::string::npos;
                        });
}
