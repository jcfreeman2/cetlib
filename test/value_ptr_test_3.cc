...
class simple_base
{
  ...
  simple_base(simple_base const& s) : i(s.i) { ++n_alive; ++n_born; }

public:
  static int n_alive;
  static int n_born;

  simple_base() : i(0) { ++n_alive; ++n_born; }
  explicit simple_base(int j) : i(j) { ++n_alive; ++n_born; }

  virtual simple_base *  clone( ) const { return new simple_base(*this); }

  virtual ~simple_base() { --n_alive; }

  bool operator==(simple_base const& o) const { return i == o.i; }
  bool isSame(simple_base const& o) const {return &o == this; }
